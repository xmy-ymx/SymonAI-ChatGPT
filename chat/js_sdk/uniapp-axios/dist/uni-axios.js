(function webpackUniversalModuleDefinition(root, factory) {
	...
	([
		...
		(function(module, exports, __webpack_require__) {
			...
			module.exports = function xhrAdapter(config) {
				return new Promise(function dispatchXhrRequest(resolve, reject) {
					...
					request.open(config.method.toUpperCase(), buildURL(config.url, config
						.params, config.paramsSerializer), true);
					...
					request.onreadystatechange = function handleLoad() {
						...
						if (request.status === 0 && !(request.responseURL && request
								.responseURL.indexOf('file:') === 0)) {
							return;
						}
					...
					if (utils.isStandardBrowserEnv()) {
						...
						// Add xsrf header
						var xsrfValue = (config.withCredentials || isURLSameOrigin(config
							.url)) && config.xsrfCookieName ?
					...
					if ('setRequestHeader' in request) {
						utils.forEach(requestHeaders, function setRequestHeader(val, key) {
							if (typeof requestData === 'undefined' && key
				...
				const uniAdapter = (config) => {
					return new Promise(function adapterUniRequest(resolve, reject) {
						uni.request({
							...
							url: buildURL(fullurl, config.params, config
								.paramsSerializer),
		...
		(function(module, exports, __webpack_require__) {
			...
			utils.forEach(['get', 'delete', 'connect', 'head', 'options', 'trace'],
